...
//     public class OpenIdAuthorizationStore<TAuthorization, TApplication, TToken, TKey> : IOpenIdAuthorizationStore<TAuthorization>
...
//         /// </summary>
//         private IRepository<TApplication, TKey> Applications;
//
//         /// <summary>
//         /// Gets the database set corresponding to the <typeparamref name="TAuthorization"/> entity.
//         /// </summary>
//         private IRepository<TAuthorization, TKey> Authorizations;
//
//         /// <summary>
//         /// Gets the database set corresponding to the <typeparamref name="TToken"/> entity.
//         /// </summary>
//         private IRepository<TToken, TKey> Tokens;
//
//         /// <inheritdoc/>
//         public virtual async ValueTask<long> CountAsync(CancellationToken cancellationToken)
//         {
//             return await ValueTask.FromResult(Authorizations.GetAll().LongCount());
//         }
//
//
//         /// <inheritdoc/>
...
//             }
//
//             return await ValueTask.FromResult(query(Authorizations).LongCount());
//         }
//
//         /// <inheritdoc/>
...
//             }
//
//             await Authorizations.AddAsync(authorization, AutoSaveChanges, cancellationToken);
//         }
//
//         /// <inheritdoc/>
...
//             try
//             {
//                 await Tokens.DeleteManyAsync(x => x.Authorization!.Id.Equals(authorization.Id), cancellationToken: cancellationToken);
//
//                 await Authorizations.DeleteAsync(authorization, AutoSaveChanges, cancellationToken);
//             }
...
//
//             var authorizations = Authorizations.GetAllMerge(x => x.Subject == subject, x => x.Application);
//             var asyncEnumerable = (from authorization in authorizations
//                 join application in Applications.GetAll() on authorization!.Id equals application.Id
//                 where application.Id!.Equals(key)
//                 select authorization).ToAsyncEnumerable();
...
//             var key = ConvertIdentifierFromString(client);
//
//             return (from authorization in Authorizations.GetAll(authorization => authorization.Application)
//                 where authorization.Subject == subject && authorization.Status == status
//                 join application in Applications.GetAll() on authorization.Application!.Id equals application.Id
//                 where application.Id!.Equals(key)
//                 select authorization).ToAsyncEnumerable();
//         }
//
...
//                 var key = ConvertIdentifierFromString(client);
//
//                 var authorizations = (from authorization in await Authorizations.GetAllAsync(cancellationToken, authorization => authorization.Application)
//                     where authorization.Subject == subject &&
//                           authorization.Status == status &&
//                           authorization.Type == type
//                     join application in Applications.GetAll() on authorization.Application!.Id equals application.Id
...
//
//             var authorizations = await Authorizations.GetAllMergeAsync(authorization => authorization.Id!.Equals(key),
//                 cancellationToken, authorization => authorization.Application);
//             return authorizations.FirstOrDefault();
//         }
//
//         /// <inheritdoc/>
...
//             //     select authorization).AsAsyncEnumerable(cancellationToken);
//
//             return Authorizations.GetAllMerge(authorization => authorization.Subject == subject,
//                 authorization => authorization.Application).Select(x => x).ToAsyncEnumerable();
//         }
//
//         /// <inheritdoc/>
...
//             {
//                 authorization = await Authorizations.GetAsync(authorization.Id, cancellationToken, entry => entry.Application);
...
//         {
//             var query = Authorizations.GetAll(authorization => authorization.Application)
//                 .OrderBy(authorization => authorization.Id!);
//
//             if (offset.HasValue)
//             {
//                 query = (IOrderedQueryable<TAuthorization>)query.Skip(offset.Value);
//             }
//
//             if (count.HasValue)
//             {
//                 query = (IOrderedQueryable<TAuthorization>)query.Take(count.Value);
//             }
//
